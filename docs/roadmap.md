...
| Feature | Explanation |
| --- | --- |
| Invocation provenance and caller idempotency | Record authenticated `session_user` and the invocation role captured before privileged execution as separate provenance fields while retaining session-user ownership until verified pooled identity exists. Add an optional request key, payload hash, retry lineage, and operator retry mode for original snapshot versus latest source; return the prior job for an exact retry and reject a changed payload under the same key |
