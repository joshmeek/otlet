...
| Feature | Explanation |
| --- | --- |
| Effective workload revisions and claim snapshots | Create one immutable revision from `input_query`, candidate SQL and sources, instruction, output schema, input shaping, decision contract, prompt-builder and validator versions, direct, cheap, and strong artifact identities, selection checks, output-affecting runtime options, sampler or deterministic mode, and relevant action-policy identities. Bind each job and claim to that revision so completion never attributes old work to mutable current rows |
...
| Versioned identity algorithms | Give persisted identities a canonical encoding and algorithm version, move freshness, contract, and mutation-authority hashes to a collision-resistant digest before the first compatibility promise, and publish native and portable test vectors. Add dual-read migration only for supported persisted state |
