...
| Terminal evidence to external archive and tombstone | Live job chain, retention policy, history choice, hold, export result, and reference dependencies | Owner-only generation fences, bounded UTC-stable manifest, external export confirmation, named blockers, nonblocking database mutation barrier, and one atomic cleanup item | Retain the complete live chain until every gate clears, then delete it exactly once and keep hash-only lifecycle and action replay tombstones |
