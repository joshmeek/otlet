...
Terminal production jobs now leave the database only through the evidence lifecycle. `successful_job_retention` defaults to null, so complete jobs are not adopted automatically until the owner enables that policy. Failed and canceled jobs use `failed_job_retention`. `evidence_max_chain_rows` defaults to 1,000 and accepts 1 through 100,000. Explicit adoption uses `request_evidence_lifecycle(...)`, while every legacy cleanup path fences due or registered chains
