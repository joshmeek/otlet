...
Portable row watches use `create_watch(..., kind => 'row')` with `mark_stale_and_enqueue`. Pair watches use bounded candidate preflight and `refresh_semantic_join_index(...)`. Source changes mark row or pair materializations stale, and committed work becomes visible to the external worker. `portable_complete_job(...)` stores the trusted output and semantic materialization in one transaction. The row and pair current-read, predicate, plan, status, export, cleanup, and audit surfaces are available in the SQL-only installation. Deletes reconcile current results; canceled work does not materialize

`portable_claim_jobs(...)` returns the shaped input snapshot, database-built prompt and prompt hash, task contract, selected model role, registered model policy, effective runtime options, evidence limits, a live claim token, and no source-table authority. A registered worker is bound to one model, so it cannot claim work for another artifact. PostgreSQL validates cheap results against the selection policy and atomically requeues rejected or schema-invalid attempts for the strong model while preserving the job ID, retry budget, and receipt history. A cheap runtime failure with no model output remains terminal, matching the native worker. Every later RPC requires the same role, worker ID, protocol version, runtime identity hash, job ID, and claim token
