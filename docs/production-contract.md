...
Portable row watches use `create_watch(..., kind => 'row')` with `mark_stale_and_enqueue`. Source inserts and updates queue bounded snapshots in the source transaction, then become visible to the external worker after commit. `portable_complete_job(...)` stores the trusted output and semantic materialization in one transaction. `semantic_index_current_rows(...)` and `semantic_index_status` expose current results and freshness. Deletes mark prior results stale; canceled work does not materialize. The portable installer rejects pair watches
