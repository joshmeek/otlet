...
Claimed jobs use `otlet.effective_job_lease_interval(...)`, which covers the task attempt timeout plus 30 seconds of completion grace. Workers pass the expected attempt when they renew a lease, store an attempt receipt, complete a job, fail a job, or use terminal recovery. Each write also requires a live lease. A reclaimed or expired worker cannot add partial trusted state. `model_selection_policy_status.effective_job_lease_interval` exposes the derived interval
