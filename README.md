...
The full demo checks row and pair watches, candidate drift, CustomScan freshness, versioned watch packs, workflow-bound `update_row`, and adversarial trust cases. It proves canonical pack diff and rollback, target binding, recommendation-only defaults, evidence holds and cleanup receipts, active-state canary deletion, receipt redaction, role grants, cancellation, model-load admission, memory pressure, cache bounds, prompt and runtime fingerprints, invariants, and Docker crash logs
