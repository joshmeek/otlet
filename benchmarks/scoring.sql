...
receipt_metrics AS (
  SELECT
    ...
    percentile_cont(0.5) WITHIN GROUP (ORDER BY ttft_ms) FILTER (WHERE ttft_ms > 0) AS p50_ttft_ms,
    percentile_cont(0.95) WITHIN GROUP (ORDER BY ttft_ms) FILTER (WHERE ttft_ms > 0) AS p95_ttft_ms,
    percentile_cont(0.5) WITHIN GROUP (ORDER BY prompt_decode_ms) FILTER (WHERE prompt_decode_ms > 0) AS p50_prompt_decode_ms,
    percentile_cont(0.95) WITHIN GROUP (ORDER BY prompt_decode_ms) FILTER (WHERE prompt_decode_ms > 0) AS p95_prompt_decode_ms,
    avg(steady_tokens_per_second) FILTER (WHERE steady_tokens_per_second > 0) AS mean_steady_tokens_per_second,
...
final_metrics AS (
  SELECT
    ...
    rm.p50_ttft_ms,
    rm.p95_ttft_ms,
    rm.p50_prompt_decode_ms,
    rm.p95_prompt_decode_ms,
    rm.mean_steady_tokens_per_second,
