...
SELECT
  ...
  s.runtime_name,
  s.task_identity_hash,
  s.source_identity_hash,
  s.model_identity_hash,
  s.runtime_options_hash,
  s.prompt_hash,
  s.input_hash,
  s.output_schema_hash,
  s.output_hash,
  s.actions_hash,
  s.portable_validation_version,
  s.prompt_tokens,
  s.generated_tokens,
  s.generate_ms,
  s.tokens_per_second,
...
SELECT
  ...
  q.dry_run_status,
  q.apply_status,
  otlet.identity_text_hash('audit_idempotency_key', q.idempotency_key) AS idempotency_key_hash,
...
SELECT
  ...
  a.dry_run_status,
  a.apply_status,
  otlet.identity_text_hash('audit_idempotency_key', er.idempotency_key) AS idempotency_key_hash,
  er.mode,
  er.status,
  er.target_name,
  er.target_table,
