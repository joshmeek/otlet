...
The install transaction runs the current SQL contract as migrations `0001` through `0070`. Re-running it skips recorded migrations and preserves existing data. This greenfield path rejects older unversioned `otlet` schemas instead of converting them
...
It installs through migration `0069`, grants existing operator and application roles plus a partial audit role, applies `0070`, and repeats the current install. The proof checks all 70 migrations, existing data and grants, review sampling, evidence-linked decisions, lifecycle, administrative-ledger, workload-acceptance and candidate-set promotion fences, entity-resolution quality decomposition, pair constraints, entity-graph conflict status and gates, bounded validators, queued ask behavior, `PUBLIC` closure, and invariants
