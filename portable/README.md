...
The worker role receives schema usage, one protocol compatibility view, and six fixed-search-path RPCs. It receives no table, source, owner, review, or action authority
...
The script creates a disposable SQL-only database, builds the worker, runs real local inference, proves the dedicated role cannot read the source table, checks trusted receipt lineage, and drops the database and role
